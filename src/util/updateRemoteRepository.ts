...
export async function updateRemoteRepository() {
  ...
  try {
    ...
    execSync(
      `gh api -X PUT -H "Content-Type: application/json" repos/${ACCOUNT}/${REPO_NAME}/contents/database/todo_list.json -F message="${DATABASE_UPDATE_MESSAGE}" -F content="${base64.toString(
        "utf-8"
